<?php
$in = 'toggle 461,550 through 564,900
...
$in = str_replace([
  ...
  'toggle ',
  ' through ',
], ['turn_off,', 'turn_on,', 'toggle,', ','], $in);
$in = explode("\n", $in);

$row = array_fill(0, 1000, FALSE);
$lights = array_fill(0, 1000, $row);

foreach ($in as $row) {
  /**
   * @var callable $command
   */
  list($command, $x_min, $y_min, $x_max, $y_max) = explode(',', trim($row));
  $command($lights, $x_min, $y_min, $x_max, $y_max);
}

$neighbors_count = 0;
foreach ($lights as $row) {
  $neighbors_count += count(array_filter($row));
}
echo $neighbors_count;

function turn_on(&$lights, $x_min, $y_min, $x_max, $y_max) {
...
function turn_off(&$lights, $x_min, $y_min, $x_max, $y_max) {
  for ($x = $x_min; $x <= $x_max; $x++) {
    for ($y = $y_min; $y <= $y_max; $y++) {
      $lights[$x][$y] = FALSE;
    }
...
function toggle(&$lights, $x_min, $y_min, $x_max, $y_max) {
  for ($x = $x_min; $x <= $x_max; $x++) {
    for ($y = $y_min; $y <= $y_max; $y++) {
