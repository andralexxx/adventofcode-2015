...
$in = preg_replace('/\\\\((?!x([0-9a-f]{2})|\")|(?=\"\n))/', '\\\\\\\\', $in);
print($in . "\n");
$hexes = preg_match_all("/\\\\x([0-9a-f]{2})/", $in, $matches);
$lines = preg_match_all("/\"\n/", $in, $matches) + 1;
$quotes = preg_match_all('/\\\\"(?!\n)/', $in, $matches);
$slashes = preg_match_all('/\\\\{2}/', $in, $matches);

$length = strlen($in) - $lines + 1;
print("\nlength: " . $length . "\n");
...
$stripped = ($hexes * 3) + ($lines * 2) + $quotes + $slashes;
print("\nresult: " . $stripped . "\n");

$in = preg_replace('/\\\\/', '\\\\\\\\', $in);
$in = preg_replace('/(?!^)"(?!$)/m', '\\\\"', $in);
$in = preg_replace('/^"/m', '"\\\\"', $in);
$in = preg_replace('/"$/m', '\\\\""', $in);
print($in . "\n");

$new_length = strlen($in) - $lines + 1;
print("\nnew length: " . $new_length . "\n");

print("\nResult: " . ($new_length - $length) . "\n");
